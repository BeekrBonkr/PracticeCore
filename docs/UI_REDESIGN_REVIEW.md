...
- [ ] **(0.11.0)** Reporting: as a non-author, right-click a public defense, Report (bell) closes the menu and asks for a reason in chat; the reporter sees "Reported …", the actions menu reads `Your report: sent`; every online moderator sees the alert with `[Review] [Play it]`; a second report from the same player says "updated" and the count stays at 1; Report is absent on your own and on private defenses; `/practice beddefense report <id> some reason` files without the prompt; a reason over `reason-max-length` is refused
